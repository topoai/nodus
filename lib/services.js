...
class Service extends EventEmitter {
    ...
    command(name, options, handler) {
        // function (name, handler) -> function (name, null, handler)
        if (arguments.length === 2 && util.isFunction(options))
            return this.command(name, null, options);
        ...
        const command = new Command(name, null, handler);
